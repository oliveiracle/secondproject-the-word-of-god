...
// Função para animar scale
function animateScale(element, show, callback) {
    if (show) {
...
function resetToMain() {
    animateScale(passageDisplay, false, () => {
        ...
        animateScale(categoryButtons, false);
    });
}
...
document.querySelectorAll('.category-title').forEach(h3 => {
    h3.addEventListener('click', () => {
        ...
        if (isVisible) {
            animateScale(subcategory, false);
        } else {
            document.querySelectorAll('.subcategory-buttons').forEach(sub => {
                animateScale(sub, false);
            });
            animateScale(subcategory, true, () => {
                Array.from(subcategory.children).forEach((btn, i) => {
                    setTimeout(() => {
                        btn.style.opacity = '0';
                        btn.style.transform = 'scale(0.8)';
                        animateScale(btn, true);
                    }, i * 100);
                });
            });
        }
...
document.querySelectorAll('.sub-btn').forEach(button => {
    button.addEventListener('click', () => {
        ...
        animateScale(categoryButtons, false, () => {
            animateScale(passageDisplay, true);
        });
    });
});
