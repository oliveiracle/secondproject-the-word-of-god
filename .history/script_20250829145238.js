...
// Evento de clique para o botão principal
startButton.addEventListener('click', () => {
    // 1. Mostra ou esconde o container de sentimentos
    feelingsContainer.classList.toggle('visible');
    
    // 2. Limpa qualquer versículo que estivesse a ser exibido
    verseDisplay.innerHTML = '';
    verseDisplay.style.display = 'none';

    // 3. Verifica se o container está agora visível para decidir se deve rolar
    if (feelingsContainer.classList.contains('visible')) {
        // Se ficou visível, espera um instante para a animação começar e depois rola suavemente
        setTimeout(() => {
            feelingsContainer.scrollIntoView({ behavior: 'smooth', block: 'start' });
        }, 100); 
    } else {
        // Se ficou invisível (clique pela segunda vez), rola de volta para o topo
        window.scrollTo({ top: 0, behavior: 'smooth' });
    }

    // 4. (Opcional) Animação de "pulso" no botão principal
...
feelingButtons.forEach(button => {
    button.addEventListener('click', () => {
        // Torna a caixa de versículos visível
        verseDisplay.style.display = 'block';

        // *** A NOSSA NOVA LINHA MÁGICA COM TIMEOUT ***
        // Espera um instante (50ms) antes de rolar, para dar tempo ao navegador de renderizar o elemento
        setTimeout(() => {
            verseDisplay.scrollIntoView({ behavior: 'smooth', block: 'center' });
