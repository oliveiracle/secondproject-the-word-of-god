...
for (let i = 0; i < feelingButtons.length; i++) {
    ...
    // 1. Esconde o container de sentimentos, caso esteja aberto
    feelingsContainer.style.display = 'none';

    // 2. Cria uma lista única com TODOS os versículos de todas as categorias
    const allVerses = Object.values(verses).flat();

    // 3. Escolhe um versículo aleatório dessa lista gigante
    const randomIndex = Math.floor(Math.random() * allVerses.length);
    const randomVerse = allVerses[randomIndex];

    // 4. Mostra a caixa de exibição e coloca o versículo lá dentro
    verseDisplay.style.display = 'block';
    verseDisplay.innerHTML = randomVerse; // Usamos innerHTML para o caso de ter <br>
});
    });
